...
func Dlerror() string {
	...
	for {
		...
		//_ = (*byte)(unsafe.Add(unsafe.Pointer(msg), length))
	...
	// we make a slice then cast to a string to force Go to copy the string from C memory into Go memory
	return string(*(*[]byte)(unsafe.Pointer(&reflect.SliceHeader{Data: msg, Len: length, Cap: length})))
}
