...
//
// [SendSuper] calls objc_msgSendSuper2 instead of objc_msgSendSuper. Unlike objc_msgSendSuper, which expects
// the superclass to message, objc_msgSendSuper2 expects the current class and begins looking the method up
// in its superclass by itself, so the receiver's own class is what belongs in superClass.
