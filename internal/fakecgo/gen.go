...
func run() error {
	...
	for _, goos := range []string{"darwin", "linux", "freebsd"} {
		...
		b.WriteString(`// Code generated by 'go generate' with gen.go. DO NOT EDIT.
...
`)
		...
		for _, sym := range libcSymbols {
			b.WriteString(fmt.Sprintf(importPragma, sym.Name, sym.Name, libcSO))
		}
		for _, sym := range pthreadSymbols {
			b.WriteString(fmt.Sprintf(importPragma, sym.Name, sym.Name, pthreadSO))
		}
		var src []byte
