...
func run() error {
	...
	for _, goos := range []string{"darwin", "freebsd", "linux", "netbsd"} {
		...
		needTrampoline := make([]Symbol, 0, len(goosSymbols))
		for _, s := range goosSymbols {
			if s.HasCalledFromGoAttr() {
				needTrampoline = append(needTrampoline, s)
			}
		}
		if len(needTrampoline) == 0 {
			continue
		}
